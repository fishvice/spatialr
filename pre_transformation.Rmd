...
When reconstructing untidy tables we sometimes may need to make a wider table again. In the above example we only made a wide table for abundance (n). We could modify the above code for the biomass (kg). Doing all in one step requires the use of `r cl("pivot_wider")`, the steps being:
